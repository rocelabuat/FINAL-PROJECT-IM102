...
const Checkout: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload GCash screenshot if payment method is GCash
      if (paymentMethod === "gcash") {
        const fileInput = fd.get("gcashProof") as File;
        if (fileInput) {
          const proofForm = new FormData();
          proofForm.append("proof", fileInput);

          await fetch(`/api/orders/gcash-proof/${order.id}`, {
            method: "POST",
